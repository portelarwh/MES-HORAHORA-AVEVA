...
const DB={db:null};
const DB_NOME='monitor-hh',DB_VER=2;
function abrir(){return new Promise((res,rej)=>{
  ...
  r.onupgradeneeded=e=>{
    ...
    ['maquinas','turnos','ajustes','catalogos'].forEach(s=>{
      if(!d.objectStoreNames.contains(s))d.createObjectStore(s,{keyPath:'id'})});
    ['dias','programacao'].forEach(s=>{
      if(!d.objectStoreNames.contains(s))d.createObjectStore(s,{keyPath:'chave'})});
    if(de&&de<2)console.info('[monitor] base migrada da versão '+de+' para '+DB_VER+' — nenhum registro alterado');
  };
